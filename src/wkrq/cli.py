...
class TableauTreeRenderer:
    ...
    def __init__(self, show_rules: bool = False, show_steps: bool = False,
                 highlight_closures: bool = False, compact: bool = False):
    ...
    def _render_node_ascii(self, node: TableauNode, lines: List[str], prefix: str, is_last: bool):
    ...
    def _render_node_unicode(self, node: TableauNode, lines: List[str], prefix: str, is_last: bool):
    ...
    def render_latex(self, tableau: Tableau) -> str:
        ...
        for node in tableau.nodes:
            if node.parent:
                ...
                lines.append(f"  \\draw[->] ({parent_id}) -- node[right] {{{rule_label}}} ({node_id});")
...
def display_result(result: TableauResult, show_models: bool = True, 
                  show_stats: bool = False, debug: bool = False):
...
Examples:
  ...
  wkrq --tree --format=unicode "p -> q"  # Unicode tree display
    ...
    parser.add_argument('--stats', action='store_true',
                       help='Show tableau statistics')
    parser.add_argument('--debug', action='store_true',
                       help='Show debug information')
    
    # Inference testing
    parser.add_argument('--consequence', choices=['strong', 'weak'], default='strong',
                       help='Type of consequence relation')
    parser.add_argument('--explain', action='store_true',
                       help='Explain inference result')
    parser.add_argument('--countermodel', action='store_true',
                       help='Show countermodel for invalid inference')
    
    # Tree visualization
    parser.add_argument('--tree', action='store_true',
                       help='Display tableau tree')
    parser.add_argument('--format', choices=['ascii', 'unicode', 'latex', 'json'], 
                       default='ascii', help='Tree display format')
    parser.add_argument('--show-rules', action='store_true',
                       help='Show rule names in tree')
    parser.add_argument('--show-steps', action='store_true',
                       help='Show step numbers in tree')
    parser.add_argument('--highlight-closures', action='store_true',
                       help='Highlight closed branches')
    parser.add_argument('--compact', action='store_true',
                       help='Compact tree display')
    parser.add_argument('--interactive', action='store_true',
                       help='Step-by-step tableau construction')
        ...
        if '|-' in args.input:
            ...
            if args.json:
                ...
                if args.tree and result.tableau_result.tableau:
                    print("\nTableau tree:")
                    renderer = TableauTreeRenderer(args.show_rules, args.show_steps,
                                                 args.highlight_closures, args.compact)
                    tree_str = render_tree(result.tableau_result.tableau, args.format, renderer)
                    print(tree_str)
...
def render_tree(tableau: Tableau, format_type: str, renderer: TableauTreeRenderer) -> str:
